...
**Current gate:** BQ-120 definition reconciliation  
...
3. `ASSESSMENT_V2.md` — approved question vocabulary and normalized values.
4. `decisions/D-118-five-axis-onboarding-assessment-contract.md` — five-axis product decision.
5. `decisions/D-119-assessment-v2-persistence-privacy-package.md` — eligibility, persistence and privacy.
6. `../../decisions/D-126-optional-guided-onboarding-handoff.md` — optional first-action handoff.
...
One product/legal boundary is deliberately open for BQ-122: when the backend is unavailable for a genuinely
...
prototype must not silently choose either behavior before owner ruling.
