...
| D-152 | SHIPPED → BQ-117 / `docs/features/account-entry/VALIDATION_RESULT.md` | Account-entry product-definition deep dive; BQ-113..117 plan approved, both HARD-STOPs deferred | Owner completed all twelve controlled-fixture scenarios and recorded PASS on 28-Aug-2026; package frozen at prototype commit `825c439`; production work requires separate bounded items |
...
| D-166 | SHIPPED → BQ-144 / `docs/features/scenarios/PRODUCTION_QA_EVIDENCE.md` | Four remaining D-148 workstreams in recommended order; Scenario/focused explorers next | Scenario production parity passed all technical gates and received owner PASS on 30-Aug-2026 |
...
| D-169 | SHIPPED → BQ-143 / production Scenario and focused-explorer results | Conditional confirmed Arya teaching handoff from eligible current Scenario results | Every eligible current result shows an exact privacy-minimised payload and requires explicit confirmation; cancel sends nothing |
...
| D-173 | READY → BQ-154 | Calculator suite is the next complete workstream | Production engines, lifecycle, evidence, candidate boundary, Arya and progression integration shipped in BQ-151..BQ-153; exhaustive production parity is next |
...
| D-175 | READY → BQ-154 | Calculator package receives owner PASS | BQ-151..BQ-153 production integration passed; exhaustive production parity and owner disposition are next |
