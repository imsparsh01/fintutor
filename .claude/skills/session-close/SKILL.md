---
name: session-close
description: Closes out a FinTutor working session by executing this repo's own session-end checklist (including D-135 delivery dispositions, D-081 archiving, and D-056 main sync) — writes docs/sessions/YYYY-MM-DD.md, verifies decision delivery state, flags spec/decision-log changes, commits, pushes, and fast-forward-merges to main. Use this whenever the user wants to wrap up, close out, end, or finish a session on this repo. Do NOT use it mid-session for a single commit — it is the full end-of-session ritual, not a generic "commit and push" helper.
---
...
- touch `CLAUDE.md`, `AGENTS.md`, `PROJECT_GOVERNANCE.md`, `DECISION_PROTOCOL.md`, or
  ...
  section),
- resolve a `main` divergence on its own (step 8 below) — that is explicitly the owner's call.
...
`docs/DECISION_LOG.md` changed this session — that fact is needed for steps 4 and 5.
...
### 3. Decision-delivery disposition check (D-135)

For every decision added or changed this session, verify that `docs/DECISION_DELIVERY_TRACKER.md` contains
exactly one current row with one of these dispositions: `NO_BUILD`, `READY`, `BLOCKED`, `DEFERRED`,
`SHIPPED`, or `SUPERSEDED`.

- `READY` and `BLOCKED` must link a BQ item.
- `DEFERRED` must name an explicit unpark condition.
- A missing or duplicate current disposition is a session-close failure; never infer `NO_BUILD` from silence.

This step verifies the delivery state already established by the session. If it requires an owner decision
that has not been made, stop and surface the gap rather than choosing a status that conceals it.

### 4. Archiving habit check (D-081)
...
### 5. Call out spec/log changes explicitly
...
### 8. Sync to `main` (D-056) — only if it's a clean fast-forward
